class PlayerSelectScreen
    def initialize
      ...
      title_text = Text.new('ASTEROIDES', size: 72, y: 40)
      title_text.x = (Window.width - title_text.width) / 2
  
      player_select_text = Text.new('SELECCIONA TU NAVE', size: 32, y: 120)
    ...
    def set_player_stat_text
      ...
      @players.each_with_index do |player, index|
        ...
        speed_text = Text.new("Velocidad - #{player.speed}%", size: 20, y: player.y + 200, color: color)
